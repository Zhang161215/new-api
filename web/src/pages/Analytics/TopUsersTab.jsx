...
const TopUsersTab = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const res = await API.get(
        `/api/analytics/top-users?days=${days}&sort=${sort}&limit=50`
      );
      if (res.data.success) {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 110,
      render: (text) => (
        <Tag color={GROUP_COLORS[text] || 'grey'} shape='circle' size='small'>
          {text || 'default'}
        </Tag>
      ),
    },
    ...
    },
    {
      ...
      width: 80,
      render: (val) => (
        <Tag color='grey' shape='circle' size='small'>{val}</Tag>
      ),
    },
  ];
    ...
    <div style={{ padding: '16px 0' }}>
      ...
      <div className='flex flex-col md:flex-row justify-between items-start md:items-center gap-2 w-full mb-4'>
        <Space>
          ...
          >
            总Token: {formatTokens(stats.totalTokens)}
          </Tag>
        </Space>
        <Space>
          <Select
